...
基于语义关系，将关键词集精确映射到度量空间，得到准确的分布关系，这是关键词识别的重要基础。

本文方法测量的距离小于直接BERT模型下的向量欧氏距离，但在相似关键词之间的距离测量中，本文方法的距离收缩率明显大于类标签之间的距离收缩率。
...
其中，W为当月正确识别的热词数量，$W\_ALL$为当月人工标记的热词数量。
...
$$
\tag{14}
RR=\frac{1}{n}\displaystyle\sum_{i=1}^nTP_i/T\_ALL_i
$$
其中，TP表示第i个热词频率的正确计数个数，$T\_ALL_i$表示手动标记热词下的所有样本的个数。

# 五、结论

1. 创新地定义了复合关键字集的距离，集合中每对匹配元素的距离权值采用归一化非线性映射，克服了距离值之间不匹配的缺点；
2. 使用BERT训练词向量比传统词向量好。
